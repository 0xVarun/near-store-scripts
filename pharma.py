...
def process(orders):
	...
	filename = '{0}-pharmacy-orders.txt'.format(datetime.date.today().strftime('%d%m%Y'))
...
def main():
	...
	ORDERS = []
	try:
		connection = psycopg2.connect("dbname=pharmacy host=db.ekastaplatform.com user=ekasta password=beacon5791 port=5432")
		cursor = connection.cursor()
		cursor.execute('''SELECT * FROM orders WHERE "storeId" = 1 AND
                        status != 'CONFIRMED' AND paid = true ORDER BY "createdAt" DESC;''')
		...
		for order in orders:
			...
			for index, product in enumerate(products):
				...
				product_O = []
				product_O.append(index + 1) # Sr No
				product_O.append(product[9]) # Name
				product_O.append(product[2]) # quantity
				product_O.append(product[17]) # price
				product_O.append(product[2] * product[17]) # total
